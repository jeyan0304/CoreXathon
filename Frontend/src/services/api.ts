...
export class RealWorkflowApiService implements IWorkflowApiService {
  ...
  async getWorkflows(): Promise<ApiResponse<Workflow[]>> { try { const payload = record(await this.request('/api/workflows'), 'workflows'); const rows = Array.isArray(payload.data) ? payload.data : []; return { success: true, data: rows.map((item) => mapWorkflowResponse(item, '').workflow) }; } catch (error) { console.warn('[getWorkflows] Backend call failed, using sample workflows:', error); return { success: true, data: INITIAL_SAMPLE_WORKFLOWS }; } }
