...
export interface IWorkflowApiService {
  ...
  startExecution(workflowId: string): Promise<ApiResponse<Snapshot>>;
  approveStep(workflowId: string, stepId: string): Promise<ApiResponse<Snapshot>>;
  rejectStep(workflowId: string, stepId: string, reason?: string): Promise<ApiResponse<Snapshot>>;
  retryStep(workflowId: string, stepId: string): Promise<ApiResponse<Snapshot>>;
...
function mapStep(raw: unknown): WorkflowStep {
  ...
  if (
    ...
    typeof step.workflow_id !== 'string' ||
    typeof step.tool_id !== 'string' ||
    typeof step.tool_name !== 'string' ||
    typeof step.status !== 'string'
  ) {
    throw new Error('Backend returned an incomplete workflow step.');
  }
  return {
    id: step.id,
    ...
    arguments: record(step.arguments, 'step arguments'),
    output: (step.output as WorkflowStep['output']) ?? null,
...
function mapSnapshot(payload: unknown, goalFallback = ''): Snapshot {
  ...
  const container = data.data === undefined ? data : record(data.data, 'API data');
  const rawWorkflow = record(container.workflow, 'workflow');
  if (
    typeof rawWorkflow.id !== 'string' ||
    typeof rawWorkflow.user_id !== 'string' ||
    typeof rawWorkflow.goal !== 'string' ||
    typeof rawWorkflow.status !== 'string' ||
    typeof rawWorkflow.created_at !== 'string' ||
    !Array.isArray(container.steps)
  ) {
    throw new Error('Backend returned an incomplete workflow snapshot.');
  }
  const steps = container.steps.map(mapStep);
  return {
    workflow: {
      id: rawWorkflow.id,
      user_id: rawWorkflow.user_id,
      goal: rawWorkflow.goal || goalFallback,
      status: rawWorkflow.status as StepStatus,
      created_at: rawWorkflow.created_at,
      updated_at: typeof rawWorkflow.updated_at === 'string' ? rawWorkflow.updated_at : undefined,
      steps,
    },
    steps,
  };
...
export class RealWorkflowApiService implements IWorkflowApiService {
  ...
  constructor(baseUrl = BASE_URL, accessTokenProvider = getAccessToken) {
  ...
  private async request(path: string, method = 'GET', body?: unknown): Promise<unknown> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(parseErrorMessage(payload, `HTTP ${response.status} from ${path}`));
      }
      return payload;
    } catch (err) {
      if (timer) clearTimeout(timer);
  ...
  private async snapshot(path: string, method = 'GET', body?: unknown): Promise<ApiResponse<Snapshot>> {
    try {
      return { success: true, data: mapSnapshot(await this.request(path, method, body)) };
    } catch (error) {
  ...
  async getTools(): Promise<ApiResponse<Tool[]>> {
    try {
      const payload = record(await this.request('/api/tools'), 'tools');
      ...
      return {
        ...
        data: rows.map((tool) => {
          ...
          return {
            ...item,
            risk_level: item.requires_approval ? 'HIGH' : 'LOW',
            status: 'ACTIVE',
          } as Tool;
        }),
      };
  ...
  async getWorkflows(): Promise<ApiResponse<Workflow[]>> {
    try {
      const payload = record(await this.request('/api/workflows'), 'workflows');
      const rows = Array.isArray(payload.data) ? payload.data : [];
      return {
        success: true,
        data: rows.map((item) => mapWorkflowResponse(item, '').workflow),
      };
    } catch (error) {
      return { success: false, error: parseErrorMessage(error) };
    }
  ...
  async getWorkflow(id: string): Promise<ApiResponse<Workflow & { steps: WorkflowStep[] }>> {
    const result = await this.snapshot(`/api/workflows/${id}`);
    return result.success ? { success: true, data: { ...result.data.workflow, steps: result.data.steps } } : result;
  }
  ...
  async createWorkflow(request: CreateWorkflowRequest): Promise<ApiResponse<CreateWorkflowResponse>> {
    try {
      return {
        success: true,
        data: mapWorkflowResponse(
          await this.request('/api/workflows', 'POST', { goal: request.goal }),
          request.goal
        ),
      };
    } catch (error) {
  ...
  async startExecution(workflowId: string) {
    return this.snapshot(`/api/workflows/${workflowId}/start-execution`, 'POST');
  }

  async approveStep(workflowId: string, stepId: string) {
    return this.snapshot(`/api/workflows/${workflowId}/steps/${stepId}/approve-action`, 'POST');
  }

  async rejectStep(workflowId: string, stepId: string, reason?: string) {
    return this.snapshot(`/api/workflows/${workflowId}/steps/${stepId}/reject-action`, 'POST', reason ? { reason } : undefined);
  }

  async retryStep(workflowId: string, stepId: string) {
    return this.snapshot(`/api/workflows/${workflowId}/steps/${stepId}/retry-step`, 'POST');
  }

  async abortWorkflow(workflowId: string): Promise<ApiResponse<{ workflow: Workflow }>> {
    const result = await this.snapshot(`/api/workflows/${workflowId}/abort-workflow`, 'POST');
    return result.success ? { success: true, data: { workflow: result.data.workflow } } : result;
  }

  async getAuditLogs(workflowId?: string): Promise<ApiResponse<AuditLog[]>> {
    try {
      const path = workflowId ? `/api/workflows/${workflowId}/audit-logs` : '/api/audit-logs';
      const payload = record(await this.request(path), 'audit logs');
      const rows = Array.isArray(payload.data) ? payload.data : [];
